@if (filled($list))
        ...
        x-data="{
            ...
            setActiveIndex({ action, index }) {
                if (action === '<') {
                    ...
                    this.activeIndex = this.activeIndex === {{ $list->count() - 1 }} ? this.activeIndex : this.activeIndex + 1;
                } else {
    ...
    {{-- Pagination --}}
  @if ($list->hasPages())
    <div class="mt-10 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-gray-600">
        <div>
            {{ __('Showing') }} {{ $list->firstItem() }} {{ __('to') }} {{ $list->lastItem() }} {{ __('of') }} {{ $list->total() }} {{ __('results') }}
        </div>
