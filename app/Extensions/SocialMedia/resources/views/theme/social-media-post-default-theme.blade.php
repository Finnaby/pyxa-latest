...
<x-card class="w-full" class:body="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4" id="summary" size="lg">
    <x-slot:head class="border-0 pb-0 pt-5">
        <div class="flex justify-between items-center">
            <h4 class="text-[17px] m-0"> @lang('Social Media Posts')</h4>
            <x-button variant="link" href="{{ route('dashboard.user.social-media.post.index') }}">
                <span class="text-foreground font-bold text-nowrap"> {{ __('View All') }} </span>
    ...
    @forelse (SocialMediaPost::query()->take(4)->get() as $post)
        <x-card class="lqd-social-media-post relative flex flex-col hover:-translate-y-1"
            ...
            <div>
                <span
                    class="relative z-2 inline-grid size-6 place-items-center rounded-lg [&_svg]:size-[18px]">
                    {!! getSocialMediaIcon($post?->getPlatformEnum()?->value) !!}
                </span>
            </div>

            <div class="lqd-social-media-post-details flex flex-col font-medium text-heading-foreground">
                ...
                <span class="mb-2.5 font-medium text-[14px] leading-4 text-foreground/50">
